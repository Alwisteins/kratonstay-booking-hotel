    ...
    <section className="w-full bg-yellow-50 py-20">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3 px-8 lg:px-16">
        {testimonial.map((testimoni) => (
          <div key={testimoni.name} className="p-6 rounded-md bg-white shadow-md space-y-4">
            <div className="flex">
            ...
            <p className="text-xs text-slate-600 italic">"{testimoni.review}"</p>
            <div>
              <h3 className="text-sm">{testimoni.name}</h3>
              <p className="text-xs text-slate-600">{testimoni.location}</p>
            </div>
          </div>
        ))}
      </div>
